classdef Tracker < handle
    ...
    %heavily modified for PSM kinetics
    ...
    methods
        ...
        function particle = Tracker(track_id,positions,quality,frames)
        ...
        function find_spot = FindNext(particle,curr_xy,next_spots,dis_error)
            if nargin <3
                dis_error = 5^2;
            end
            find_spot = [false -1];
            post_leng = size(next_spots,1);
            x = curr_xy(1);
            y = curr_xy(2);
            ...
            for i = linspace(1, post_leng, post_leng)
                dis = (x-next_spots(i,1))^2 + (y-next_spots(i,2))^2;
                if dis < near
                    near = dis;
                    next_x = next_spots(i,1);
                    next_y = next_spots(i,2);
                    find_spot(2) = i;
                end
            end
...
%             velocity = PolynomialPredict(particle);
            velocity = [0,0];
            x = particle.positions(end,1) + velocity(1);
            y = particle.positions(end,2) + velocity(2);
            near = 9999999;
            for i = linspace(1, post_leng, post_leng)
...
%                 error('Velocity calculation should be started with at least the 2nd frame.');
                ...
                previous_xy = particle.positions(:,1:2);
                velocity = [0,0];
        ...
        %Input    particle                : tracker, refer to Tracker function particle = Tracker(track_id,positions,quality,frames)
        %          
        %         all_full_images         : array, size (#frames,h,w), 
        %         n                       : int, the parttern is average over the last n frame from the current time
        %         region                  : w*h array, pattern range = x-h/2+1:x+h/2, y-w/2+1:y+w/2, x, y is position in particle.positions
        %Output:  pattern                 : has the same size with region
            ...
            xxs = ceil(particle.positions(:,2));%pay attention to here xy
            yys = ceil(particle.positions(:,1));
            t = 0;
            
            while t < n && t < size(particle.positions,1)
        ...
        %MyPattern: return pattern of the particle, (averaged over the first n patterns) 
            ...
            xxs = floor(particle.positions(:,1));%pay attention to here xy
            yys = floor(particle.positions(:,2));
            x = xxs(end,1);
...
%                pass
        ...
        % for odd side
            ...
            w = size(region,2)-1;
            image_size = size(one_image);
            xxs = round(particle.positions(:,2));%pay attention to here xy
            yys = round(particle.positions(:,1));
            x = xxs(end,1);
            y = yys(end,1);%pay attention to here xy
...
%                pass
        ...
        % for odd side
            ...
            w = size(region,2)-1;
            image_size = size(one_image);
            xxs = round(particle.positions(f,2));%pay attention to here xy
            yys = round(particle.positions(f,1));
            x = xxs(end,1);
            y = yys(end,1);%pay attention to here xy
            if x-h/2 <= 0 || x+h/2 > image_size(1) || y-w/2 <= 0 || y+w/2 > image_size(2)
%                pass
        ...
        %         paper reference: Three-dimensional localization microscopy in live flowing cells
            ...
            particle.positions(end,4) = position_z;
        end
        ...
        function SavePatterns(particle, all_full_images, region, filename)
            ...
            patterns = zeros([size(particle.positions,1) size(region)]);
            h = size(region,1);
            w = size(region,2);
            image_size = size(squeeze(all_full_images(1,:,:)));
            xxs = ceil(particle.positions(:,2));%pay attention to here xy
            ...
            for t = 1:size(particle.positions,1)
...
%             x = particle.positions(end,1);
%             y = particle.positions(end,2);
