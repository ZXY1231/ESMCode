tic
trackmate_spots = [ "D:\20211017\trackmate\5nmIgG_5nmIgA_4ms_200fps_1fold_avg10_diff_img_seletion_8_5_2\All Spots statistics.csv"];
...
for folder = 1:1
    folder
    clear data1 data01
    table_name = trackmate_spots(folder);
    ...
    for i = 1:size(all_spots,2)
        ...
        if length(x) >6
            ...
            max_supress = 1;
            for k = 2:length(x)/2      % x here is the x axis of full metrix
    ...
    %%
    % save('05_10min_done_trackers_declose_spots_6_028')

    %%
    intensity = zeros(size(done_trackers,2),5);
    for i = 1:size(done_trackers,2)
        if size(done_trackers{i}.positions,1)<1
            continue
        end
    ...
    %%
    % save('05_10min_intensity_declose_spots_6_028')
    ...
    intensity(:,1) = intensity(:,1)+(folder-1)*88200; % correct frames
    intensity_total_min = cat(1,intensity_total_min, intensity);
    toc
end
%% plot intensity counts f(t)

range = [2.4, 2.8, 4, 100];
counts = zeros(3,940*15);
for i = 1:size(intensity_total_min,1)
    particle = intensity_total_min(i,:);
    qua = particle(1,5);
    if qua>range(4)
        continue
    elseif qua>=range(3)
...
time_window = 940/2;
counts_window = zeros(3,30);

for i = 1:30
    t = ((i-1)*time_window+1:i*time_window);
    counts_window(:,i) = sum(counts(:,t),2);
end
figure(21)
hold on
plot(0.5:0.5:15,counts_window(1,:))
plot(0.5:0.5:15,counts_window(2,:))
plot(0.5:0.5:15,counts_window(3,:))
hold off
title('counts along the time')
axis([1 15 0 inf])
xlabel('time /min')
ylabel('counts')
legend('IgG', 'IgA', 'Complex')

%%
Kd = zeros(1,60);
f_conv_all = zeros(1,60);
for t = 1:60
...
% %%
% figure(19)
% scatter(intensity(:,4),intensity(:,1))
% % axis([0 80 0 20])